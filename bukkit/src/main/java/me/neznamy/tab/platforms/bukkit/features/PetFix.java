...
public class PetFix extends TabFeature {
	...
	private int getPetOwnerPosition() {
		if (AdapterProvider.getMinorVersion() >= 17) {
			//1.17.x, 1.18.x
			return 18;
		} else if (AdapterProvider.getMinorVersion() >= 15) {
			//1.15.x, 1.16.x
			return 17;
		} else if (AdapterProvider.getMinorVersion() >= 14) {
			//1.14.x
			return 16;
		} else if (AdapterProvider.getMinorVersion() >= 10) {
			//1.10.x - 1.13.x
			return 14;
		} else {
			//1.9.x
	...
	public boolean onPacketReceive(TabPlayer sender, Object packet) throws ReflectiveOperationException {
		if (AdapterProvider.get().isInteractPacket(packet)) {
			...
			if (AdapterProvider.get().isInteractionAction(packet)) {
	...
	public void onPacketSend(TabPlayer receiver, Object packet) throws ReflectiveOperationException {
		if (AdapterProvider.get().isMetadataPacket(packet)) {
			Object removedEntry = null;
			List<Object> items = AdapterProvider.get().getMetadataEntries(packet);
			if (items == null) return;
			for (Object item : items) {
				if (item == null) continue;
				if (AdapterProvider.get().getMetadataSlot(item) == petOwnerPosition) {
					final Object value = AdapterProvider.get().getMetadataValue(item);
					if (value instanceof java.util.Optional || value instanceof com.google.common.base.Optional) { // < 1.13 used Google's Optional
						removedEntry = item;
					}
				}
			}
			if (removedEntry != null) items.remove(removedEntry);
		} else if (AdapterProvider.get().isSpawnLivingEntityPacket(packet) && AdapterProvider.getMinorVersion() < 15) { // < 1.15
			DataWatcher watcher = AdapterProvider.get().getLivingEntityMetadata(packet);
			...
			if (petOwner != null && (petOwner.getValue() instanceof java.util.Optional || petOwner.getValue() instanceof com.google.common.base.Optional)) {
				...
				AdapterProvider.get().setLivingEntityMetadata(packet, watcher);
			}
		}
	}
